...
export class FingeringChart {
  ...
  drawFretboard(): void {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      this.ctx.lineTo(
        this.fretboardOriginX + i * this.stringSpacing,
        this.fretboardOriginY + this.fretboardHeight,
      )
      this.ctx.closePath()
      this.ctx.stroke()
    }
    ...
    for (let i = 0; i <= this.nFrets + 1; i++) {
      ...
      this.ctx.lineTo(
        this.fretboardOriginX + this.fretboardWidth,
        this.fretboardOriginY + i * this.fretSpacing,
      )
      this.ctx.closePath()
      this.ctx.stroke()
      ...
      this.ctx.fillText(
        (this.startFret + i).toString(),
        this.stringSpacing / 2,
        this.fretboardOriginY + (i + 0.5) * this.fretSpacing,
      )
    }
  }
  ...
  getSingleHitbox(single: Single): [number, number, number] {
    ...
    const y =
      this.fretboardOriginY +
      this.fretSpacing / 2 +
      (single.fret - this.startFret) * this.fretSpacing
  ...
  drawSingles(): void {
    ...
    for (const singlesIdx in this.singles) {
      ...
      let color: string
      if (
        this.highlighted.some(
          (highlighted) => highlighted.string === single.string && highlighted.fret === single.fret,
        )
      ) {
        color = 'red'
  ...
  drawBarres(): void {
    for (const barresIdx in this.barres) {
      ...
      const barreStartCenterX =
        this.fretboardOriginX + guitarStringToZeroOffset(barre.startString) * this.stringSpacing
      const barreCenterY =
        this.fretboardOriginY +
        this.fretSpacing / 2 +
        (barre.fret - this.startFret) * this.fretSpacing
      ...
      this.ctx.arc(
        barreStartCenterX + barreWidth * this.stringSpacing,
        barreCenterY,
        barreRadius,
        -Math.PI / 2,
        Math.PI / 2,
      ...
      if (labelFinger) {
        ...
        this.ctx.fillText(
          ...
          barreStartCenterX + (barreWidth * this.stringSpacing) / 2,
          barreCenterY,
        )
      }
    }
